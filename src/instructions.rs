...
fn prelude(origin:(f64,f64)) -> (Context,Context,Context,Context,Context) {
    ...
    let stroke = StrokeContext::new(20.0);
...
pub fn do_this(mut doc:svg::Document, origin:(f64,f64)) -> svg::Document {
    ...
    let Poi = POrd::new(300.0,PI/3.0, &svg_origin);
    ...
    doc = basic::circle(doc, &Pord(Poi.clone()),300.0, &prime_ctx);
    basic::arc_path(doc,10.0,&Pord(Poi.clone()),&Cord(0.0,-300.0),300.0,true,&path_ctx)
}